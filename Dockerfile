...
# compile assets with verbose output to debug any issues
RUN mix assets.deploy --verbose
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/trading_fours ./
